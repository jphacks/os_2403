  ...
  const fetchRooms = async () => {
    try {
      if (accountType === "user") {
        ...
        const response = await apiClient.get(`scoutlist/getcommunitydetail`, {params: { user_uuid },});
        console.log(response)
        setRooms(response.data);
      }
      else if(accountType === "community") {
        const community_uuid = currentCommunity?.uuid;
        const response = await apiClient.get(`scoutlist/getuserdetail`, {params: { community_uuid },});
        console.log(response)
        setRooms(response.data);
      }
    } catch (error) {
          ...
          {rooms.map(room => (
            ...
            >
              <Avatar className="chat-room-avatar">
                <AvatarImage src={room.detail_info?.img || 'https://kotonohaworks.com/free-icons/wp-content/uploads/kkrn_icon_user_4.png'} />
